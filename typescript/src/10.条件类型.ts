/* 
  TS中的基础类型
  string number boolean null undefined void（描述函数的返回值）
  数组、元组、枚举（常量枚举、普通枚举）
  never 所有类型的子类型（底端类型）如果这个值我不需要 可以将这个属性重置为never 完整性保护
  any 顶端类型 最大的类型
  object 非原始类型 对象、函数、数组
  bigInt symbol 类型
  包装类型 String Number Boolean Object ... 描述的是实例（什么是包装对象）
  字面量类型 具体的值当做类型
  ...
  函数重载 function 伪重载（只是对类型的重载，输入和输出 存在着某种关联，参数的个数不一致 实现的逻辑也不一致）

  type X = keyof 索引类型查询（联合类型）
  typeof 类型查询
  ...
  方法 原型方法和实例方法 原型方法：eat():void 实例方法：eat:()=>void
  void类型表示不关心返回值

  type（主要是用于 | & 别名，编写一些复杂类型 可以使用type 条件类型 keyof typeof）
  interface（描述形状的，extends，implements）抽象的

  接口 ? 可选属性 readonly 只读的 任意属性[key:string]:any 可索引属性[key:number]:any
  ...
  内置条件类型 exclude extract 基于条件实现的

  类型推断 inference infer 推断
  infer关键字只能用在条件类型中 用来提取类型的某一个部分的类型 放在不同的位置 就可以帮我们取不同位置的类型
*/
...
type PromiseReturnValue = PromiseV<PromiseV<Promise<number>>> // number
